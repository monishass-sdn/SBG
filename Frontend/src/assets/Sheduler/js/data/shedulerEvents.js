...
const standbyBookingMessage = "You are trying to make a standy by day booking!Do you want to continue?";
...
const before_Launch_date_message = "You are trying to make a booking before this boat's launch date.Do you want to continue";
...
const before_pre_launch_date_message = "You are attempting to book this boat before its pre-launch date. Please select a date after the pre-launch date.";
...
const select_boat_name_message = "Please select the name of the boat.";
...
const specialDayMissing = "Special day booking can only be booked as a set.";


//const previousDayMessage = "you are select previous day. current date "+timeszon_Set+ " is ";
const previousDayMessage = "A booking cannot be created for a previous day";
...
$(document).on("click","[data-schedule-id]",function() {
    ...
    $(document).on("click",".tui-full-calendar-weekday-grid-line",function() {
        ...
        // setTimeout(() => {
        //     let newDate_Start = $("#tui-full-calendar-schedule-start-date").val();
        // let newDate_End =   $("#tui-full-calendar-schedule-end-date").val(); 
    
        // let start1 = newDate_Start.split(" ");
        // let end = newDate_End.split(" ");
    
        // if(start1.length == 2){
        //     $("#tui-full-calendar-schedule-start-date").val(start1[0]);
        //     $("#tui-full-calendar-schedule-end-date").val(end[0]);
        //     if(end[1] == '00:30' || end[1] == '00:00'){
        //         $("#tui-full-calendar-schedule-end-date").val(start1[0]); 
        //     }
        // }
        //     console.log("this is the third message")}, 400);
   ...
   //End..................................................
    ...
    function getFormattedDate_Friday_only(dateVal) {
       ...
       debugger;
        ...
        //     }           
        //     }
        if(currentDay == 'Fri' && hor[0] >= 17){
            return true;
        } else if(currentDay == 'Sun' && 12 >= hor[0]){
            return true;
    ...
    function fun_weekDayCount_AddZeroDay(count){
       ...
       debugger;
        console.log(MomentConvert_currentDay_Date());
        console.log(momment_newDate_convert_YYYY_MM_DD(MomentConvert_currentDay_Date()));
...
$.each(shedulerList_tmp, function (key, val) {
    if(val.isThisAOwnerCanceledStandByBooking != true){
...
function removeTime_To_Sheduler(){
debugger;
    let newDate_Start = $("#tui-full-calendar-schedule-start-date").val();
    let newDate_End =   $("#tui-full-calendar-schedule-end-date").val(); 
...
function You_are_booking_Special_day_changes_1(commenDatas){
    ...
    // sibi testing.......... 29..12..2021

      obj = Back_To_Back_Booking_Calculation(obj);

       if(obj.Back_to_Back_Booking == true)
        {
                if(obj.isAStandByBooking == true)
                {
                    if(obj.Back_to_Back_id == 3)
    ...
    { 
        if(obj.editShduler_frondent == true){
...
function AddShedule_directCalling(obj){
...
function EditShedule_DirectCalling(obj){
                            ...
                            if(datas.status == true)
                            {
                                ...
                                {                   
                                    var count_Temp =  parseInt(datas.LOAInWeekday) + parseInt(datas.LOAInWeekend);
                                        ...
                                        $("#P-IsLOAIncluded").text("You are about to partially cancel this booking, which will result in the loss of "+count_Temp+" day/s from this booking. Are you sure you want to continue?");
...
function bookingDaysinArray(obj){

    let _start = new Date(momment_newDate_convert_YYYY_MM_DD(obj.start));
    let _end = new Date(momment_newDate_convert_YYYY_MM_DD(obj.end));

    let tmp_month_f = _start.getMonth();
    let tmp_year_f = _start.getFullYear();
    let tmp_Date_f = _start.getDate();
    let date_f = new Date(tmp_year_f, tmp_month_f, tmp_Date_f);

    let allBookedDate = [];
    let retunConvertedDate = [];

        do{

        //debugger;

            let _pushdata = date_f.toString();
            allBookedDate.push(_pushdata);

           

            date_f.setDate(date_f.getDate() + 1);
            var tmp_Add_Date_f = new Date(momment_newDate_convert_YYYY_MM_DD(date_f));

        }while(tmp_Add_Date_f <= _end);

        
        $.each(allBookedDate, function (key, val) {

            let _values = new Date(momment_newDate_convert_YYYY_MM_DD(val));
            retunConvertedDate.push(_values.toString());

        });

        return retunConvertedDate;
...
function continueSpecialDay_bookingChekOneTime(obj){
debugger;
   
    var date_specialday = JSON.parse(sessionStorage.getItem("load_SpecialDays"));
    var _start = new Date(momment_newDate_convert_YYYY_MM_DD(obj.start));
    var _end = new Date(momment_newDate_convert_YYYY_MM_DD(obj.end));
    var _baseArrayDate = [];
    var _bookingDateInArray = bookingDaysinArray(obj); //[];
    var bookedArry;
    ...
    $.each(date_specialday, function (key, val) {
        ...
        var special_startDate = new Date(momment_newDate_convert_YYYY_MM_DD(val.Start_Date));
        var special_endDate = new Date(momment_newDate_convert_YYYY_MM_DD(val.End_Date));

       var tmp_month_f = special_startDate.getMonth();
       var tmp_year_f = special_startDate.getFullYear();
       var tmp_Date_f = special_startDate.getDate();
       var date_f = new Date(tmp_year_f, tmp_month_f, tmp_Date_f);
       ...
       do{
        ...
        //debugger;
            ...
            var _pushdata = date_f.toString();

            _childArrayDate.push(_pushdata);

            date_f.setDate(date_f.getDate() + 1);
            var tmp_Add_Date_f = new Date(momment_newDate_convert_YYYY_MM_DD(date_f));

        }while(tmp_Add_Date_f <= special_endDate);


        _baseArrayDate.push(_childArrayDate);
    ...
    $.each(_baseArrayDate, function (key, val){ 
        
        var _countItems = 0;
        var _countOfItemBooked = 0;

        $.each(val, function (key2, val2){
          //debugger;
          let convert_Date = new Date(momment_newDate_convert_YYYY_MM_DD(val2)); 
          let convert_Date_2 = convert_Date.toString();
          
          _countItems = _countItems + 1;
          
          if(jQuery.inArray(convert_Date_2, _bookingDateInArray) != -1){
           // console.log("is in array");
            _countOfItemBooked = _countOfItemBooked + 1;
            bookedArry = val;
          } 
          else {
               // console.log("is NOT in array");
        ...
        if(_countOfItemBooked != 0){
           

             countOfFirst = _countItems;
             countOfSecond = _countOfItemBooked;            
    ...
    if(countOfFirst == countOfSecond){
         ...
         obj.frondent_chek_continueSpecialDay =  true;

         return obj;

    }
    else{
         obj.frondent_chek_continueSpecialDay = false;
         obj.frondent_check_SpecialDay_array = bookedArry;

         obj = re_edit_bookingDate_by_specialDayBooking(obj);
...
function re_edit_bookingDate_by_specialDayBooking(obj){

    debugger;

    let bookingDateInArray = bookingDaysinArray(obj);
    let length_bookingDate = bookingDateInArray.length;
    let length_bookingDate_cal = length_bookingDate - 1;

    let selected_specialDay = obj.frondent_check_SpecialDay_array;
    let length_specialDay = selected_specialDay.length;
    let length_specialDay_cal = length_specialDay - 1;

    let selected_BookingDay_0 = new Date(momment_newDate_convert_YYYY_MM_DD(bookingDateInArray[0]));
    let selected_BookingDay_last = new Date(momment_newDate_convert_YYYY_MM_DD(bookingDateInArray[length_bookingDate_cal]));

    let selected_Specialday_0 = new Date(momment_newDate_convert_YYYY_MM_DD(selected_specialDay[0]));
    let selected_Specialday_last = new Date(momment_newDate_convert_YYYY_MM_DD(selected_specialDay[length_specialDay_cal]));
  
    let current_ausDate = MomentConvert_currentDay_Date();  
    let currentDates = getFormattedDate_WithOut_Zero_Time(current_ausDate);
    let selspecialday = getFormattedDate_WithOut_Zero_Time(selected_Specialday_0);
    console.log(selspecialday);
    console.log(currentDates);
  //  if(selected_BookingDay_0 <= selected_Specialday_0){
       

  //  }
    if(selected_Specialday_0 <= selected_BookingDay_0){
        var start_str = new Date(momment_newDate_convert_YYYY_MM_DD(selected_Specialday_0.toString())); 
        start_str = new Date(start_str.setHours(conver_Hours("09:00"),conver_Minit("09:00"),00,0));     
        obj.start = start_str.toString();

    }
    else{
        if(currentDates == selspecialday){
            var current_Time = Moment_Time_Onley(current_ausDate) //Time_Onley(currentDay_tmp);//current_Time
             if("12:00:00" >= current_Time){
                var start_str = new Date(momment_newDate_convert_YYYY_MM_DD(selected_Specialday_0.toString())); 
                start_str = new Date(start_str.setHours(conver_Hours("09:00"),conver_Minit("09:00"),00,0));     
                obj.start = start_str.toString();//selected_Specialday_0.toString();
    ...
    console.log("code on going");
    if(selected_Specialday_last <= selected_BookingDay_last ){
        var start_str = new Date(momment_newDate_convert_YYYY_MM_DD(selected_Specialday_0.toString())); 
        start_str = new Date(start_str.setHours(conver_Hours("09:00"),conver_Minit("09:00"),00,0));     
        obj.start = start_str.toString();
    }
      ...
      //  var end_str   = new Date(momment_newDate_convert_YYYY_MM_DD(selected_Specialday_last.toString()));
      var end_str   = new Date(momment_newDate_convert_YYYY_MM_DD(selected_Specialday_last.toString()));
      end_str = new Date(end_str.setHours(conver_Hours("17:00"),conver_Minit("17:00"),00,0)); 

        obj.end = end_str.toString(); //selected_Specialday_last.toString();
    }


    obj = WeekEnd_Weekday_TotalDayCount_Calculator(obj);
...
function WeekEnd_Weekday_TotalDayCount_Calculator(obj){

debugger;
    const Temp_Date_start = new Date(momment_newDate_convert_YYYY_MM_DD(obj.start));                        
    const Temp_Date_end = new Date(momment_newDate_convert_YYYY_MM_DD(obj.end));
    var Temp_Date_dateDiff= Math.round((Temp_Date_end - Temp_Date_start)/(1000*60*60*24));
    var Temp_Date_Winter_dateDiff = Temp_Date_dateDiff+1;
    const Temp_Date_sundays = Math.floor((Temp_Date_Winter_dateDiff + (Temp_Date_start.getDay() + 6) % 7) / 7);
    const Temp_Date_weekenddays = 2 * Temp_Date_sundays + (Temp_Date_end.getDay()==6) - (Temp_Date_start.getDay()==0);                        
...
if(obj.Is_StandByBooking){
    let current_ausDate = MomentConvert_currentDay_Date();
         
    var currentDates = getFormattedDate_WithOut_Zero_Time(current_ausDate);
    
    var startDateConvertDate = getFormattedDate_WithOut_Zero_Time(MomentConvert(obj.start)); 
   ...
   if(currentDates == startDateConvertDate){
        ...
        if("12:00:00" >= current_Time){
                ...
                if(2 <= Temp_Date_weekdays){
                    ...
                    obj.WeekDay_Count = fun_weekDayCount_AddZeroDay(Temp_Date_weekdays);
                   }
               else{
                obj.WeekDay_Count = 0;
                   }
       
                   obj.WeekEnd_Count = fun_weekendCount_AddZeroDay(Temp_Date_weekenddays)
    ...
    if("12:00:00" <= current_Time){                
        ...
        if(2 <= Temp_Date_weekdays){
            ...
            obj.WeekDay_Count = fun_weekDayCount_AddOneDay(Temp_Date_weekdays);
        }
        else{
            obj.WeekDay_Count = 0;
        }
    
        obj.WeekEnd_Count = fun_weekendCount_AddOneDay(Temp_Date_weekenddays)
                                    
       // objects.Temp_Date_dateDiff = (parseInt(objects.Temp_Date_weekdays) + parseInt(objects.Temp_Date_weekenddays)) - 1;
    
    
    }
    obj.TotalDay_Count =  obj.WeekDay_Count + obj.WeekEnd_Count;
...
function Finalize_One_AddSheduel_API_Call(obj){
    ...
    if(public_Global_unavilable == false){
       ...
       // var check_SpecialDay = Object();
       // check_SpecialDay.multiple_specialDay_chek == false
       // check_SpecialDay.specialday_check == false
           ...
           // if(check_SpecialDay.multiple_specialDay_chek == false){
                ...
                if(check_SpecialDay.specialday_check == false || pageIdentiFication == "AdminBooking" || pageIdentiFication == "boat-maintenance" )
                {
                    ...
                    //this to start..... 10/dec/2021...........

                    obj = continueSpecialDay_bookingChekOneTime(obj);

                    debugger;

                   if(obj!= undefined) {
                   
                    if(obj.frondent_chek_continueSpecialDay == true)
                    {
                        ...
                        let messag = You_are_booking_Special_day + " " + specialDayMissing;
                        
                        Commen_ConformationMessage_Therd(messag,1,passJson);

                        //CommenMessage(specialDayMissing);
        ...
        */
            ...
            if(_currentDate > _startDateConvert){
               ...
               // CommenMessage(previousDayMessage + getFormattedDate_WithOut_Zero_Time(MomentConvert_currentDay_Date())); 
                CommenMessage(previousDayMessage); 
            }
        }
