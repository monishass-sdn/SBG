...
@Component({
  selector: 'app-view-owner-all-boat-view',
  templateUrl: './view-owner-all-boat-view.component.html',
  styleUrls: ['./view-owner-all-boat-view.component.css']
})
export class ViewOwnerAllBoatViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    sessionStorage.setItem("realoding_end","1");
    ...
    this.data = JSON.parse(sessionStorage.getItem('ownerData_view_owner_all_boat_View')); 
    this.getBoteByOwner();
           ...
           {           
            ...
            if(val.User_Id == ownerid)
            {
               ...
               if(val.Cancelled_Status == 0){
                bookingStatus = "<span style='color: green;font-size: large;'>Confirmed booking</span>";
               }
               else if(val.Cancelled_Status == 1){
                
                bookingStatus = "<span style='color: red;font-size: large;'>Cancelled booking</span>";

               }
               else
               {
                bookingStatus = "<span style='color: orange;font-size: large;'>Processing</span>";
               }
                 
   
               if(firstChek == 0){              
                 
                 bindingTableData ='<tr><td>'+boat_Name+'</td><td>'+startdate+'</td><td>'+finishDate+'</td><td>'+bookingStatus+'</td></tr>';
